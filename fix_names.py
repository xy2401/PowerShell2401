...
root_dir = r"d:\xy2401\codePublic\powershell2401"
...
for root, _, files in os.walk(root_dir):
    for filename in files:
        if filename.endswith('.ps1') or filename.endswith('.psm1'):
            ...
            new_text = re.sub(r'(?i)\bCreate-Directories\b', 'New-Directories', text)
            new_text = re.sub(r'(?i)\bGet-Directory-Depth\b', 'Get-DirectoryDepth', new_text)
            new_text = re.sub(r'(?i)\bInit-Log\b', 'Initialize-Log', new_text)
            new_text = re.sub(r'(?i)\bLog-Message\b', 'Write-LogMessage', new_text)
