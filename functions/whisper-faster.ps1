...
Write-LogMessage -message "=> [Faster-Whisper] 开始处理视频字幕生成，共选中 $($targetDirs.Count) 个目录"
...
foreach ($dir in $targetDirs) {
    ...
    foreach ($file in $files) {
        ...
        if (-not [string]::IsNullOrWhiteSpace($Language)) {
            ...
            Write-LogMessage -message " [!] 未指定语言，将由 Whisper 自动检测"
        }
        ...
        try {
            ...
            if ($LASTEXITCODE -eq 0) {
                ...
                Write-LogMessage -message " [Error] faster-whisper 返回退出码: $LASTEXITCODE" -Level Error
            }
        }
        catch {
...
Write-LogMessage -message "=> [Faster-Whisper] 全部任务处理完成！"
