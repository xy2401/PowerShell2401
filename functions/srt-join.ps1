...
foreach ($baseName in $baseNames) {
    ...
    foreach ($l in $Lang) {
        ...
        if (Test-Path -LiteralPath $path1) {
            ...
            $hasAll = $false
            Write-LogMessage "找不到组 $baseName 对应的语言字幕: $l" -Level Warning
            break
        }
    }
    ...
    Write-LogMessage "合并字幕组: $baseName" -Level Info
    
    $allSubtitles = @()
    foreach ($l in $Lang) {
        Write-LogMessage "  -> 提取: $(Split-Path $langFiles[$l] -Leaf)" -Level Info
        $allSubtitles += Parse-Srt -FilePath $langFiles[$l] -LangTag $l
    }
    
    Write-LogMessage "  -> 总共有 $($allSubtitles.Count) 条字幕内容参与合并..." -Level Info
                ...
                foreach ($other in $allSubtitles) {
                    if ($other.Lang -eq $otherLang -and -not $processed.Contains($other)) {
                        ...
                        if ($isMatch) {
                            ...
                            Write-LogMessage "[Step1: 1:1匹配] 第一轨($($item.Lang) 序号$($item.Index)) 及 匹配轨($($other.Lang) 序号$($other.Index)) (IoU面积比例: $perc%)" -Level Info
                            break
                        }
                    }
                }
            ...
            foreach ($cItem in $currentCluster.Items) {
                if ($cItem.Lang -ne $sub.Lang) {
                    ...
                    if ($crossTime -ge -($Tolerance)) {
                        ...
                        Write-LogMessage "[Step2: 链式缝合并发] -> 目标句($($sub.Lang) 序号$($sub.Index)) 与链内句($($cItem.Lang) 序号$($cItem.Index)) 产生 $gapType" -Level Info
                        break
                    }
                }
            }
    ...
    Write-LogMessage "  -> 合并完毕，共生成 $($exactClusters.Count) 个极准 1:1 结构，与 $($chainedClusters.Count) 个拼接混合群组 (容错: ${Tolerance}s, 拼接上限: $MaxMergeCount)" -Level Info
    ...
    for ($i = 0; $i -lt $clusters.Count - 1; $i++) {
        ...
        if ($overlapSecs -gt 0) {
            ...
            if ($overlapSecs -le ($prevMaxYield + $nextMaxYield)) {
                ...
                Write-LogMessage "[Step4: 边缘修剪] 群组 $i 与 群组 $($i+1) 轻微重叠 $([math]::Round($overlapSecs, 3))s -> 完美分离 (前级削去 $([math]::Round($prevCut, 3))s, 后级削去 $([math]::Round($nextCut, 3))s)" -Level Info
            } else {
                ...
                Write-LogMessage "[Step4: 边缘修剪] 群组 $i 与 群组 $($i+1) 大幅重叠 $([math]::Round($overlapSecs, 3))s -> 仅作极值退让 (前级极值 $([math]::Round($prevMaxYield, 3))s, 后级极值 $([math]::Round($nextMaxYield, 3))s)" -Level Warning
            }
    ...
    Write-LogMessage "  -> 已保存至合并文件: $outFileName" -Level Success
}
